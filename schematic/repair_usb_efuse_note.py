#!/usr/bin/env python3
"""Move the clipped USB eFuse settings note into the readable centre of box 1."""
...
from wire_usb_efuse_support import NOTE


REJECTED = "canonical-usb-efuse-support-wire-2026-08-28"
TX = "canonical-usb-efuse-note-anchor-repair-2026-08-28"
...
def prepare() -> int:
    ...
    intended = "Repair the rejected box-1 eFuse settings note by centring the complete note at x=450 y=3950"
    base.begin_transaction(
        base.MUTATION_STATE, base.MUTATION_LEDGER, transaction_id=TX,
        project_uuid=PROJECT, document_uuid=PAGE, scope="POWER_ENTRY", stage="text",
        kind="repair", repairs_transaction_id=REJECTED, intended_delta=intended,
        snapshot_path=snapshot, expected_checks=[
            "the complete settings note is visible inside box 1",
            "the note does not overlap U1 or the support network",
            "the UVLO OVLO PG ILIM and ITIMER values are readable",
            "no electrical record component or unrelated text moves",
        ],
    )
    all_rows = rows(before["source"])
    notes = [record for record in all_rows if record[0] == "TEXT" and len(record) > 5 and record[5] == NOTE]
    if len(notes) != 1:
        raise SystemExit(f"expected one eFuse note, found {len(notes)}")
    notes[0][2] = 450
    notes[0][3] = 3950
    payload = {"source": "\n".join(json.dumps(record, separators=(",", ":")) for record in all_rows),
               "expectedSourceHash": before["source_hash"], "expectedDocumentUuid": PAGE,
...
def record() -> int:
    ...
    if len(notes) != 1 or notes[0][2:4] != [450, 3950]:
        raise SystemExit(f"note read-back mismatch: {notes}")
    semantic = JOBS / f"{TX}-semantic.json"
    semantic.write_text(json.dumps({
        "schema_version": 1, "transaction_id": TX, "project_uuid": PROJECT,
        "document_uuid": PAGE, "scope": "POWER_ENTRY", "stage": "text",
        "intended_delta": "Repair the rejected box-1 eFuse settings note by centring the complete note at x=450 y=3950",
        "pre_source_hash": before["source_hash"], "post_source_hash": after["source_hash"],
        "saved": True, "affected": ["USB eFuse settings note"],
        "note_position": [450, 3950], "census": after["census"],
