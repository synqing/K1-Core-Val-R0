#!/usr/bin/env python3
"""Wire the source-derived USB/input TPS259474L support network in box 1."""
...
TX = "canonical-usb-efuse-support-wire-2026-08-28"
PAYLOAD = JOBS / f"{TX}-payload.json"
NOTE = "U1 SET: UVLO≈4.17V | OVLO≈5.46V | PG≈4.49V | ILIM≈2.51A | ITIMER NC"
CONNECTIONS = {
    "U1-PWR1": {"1": "USB_EFUSE_EN", "2": "USB_EFUSE_OVLO", "3": "USB_EFUSE_PG",
                "4": "USB_EFUSE_PGTH", "7": "USB_EFUSE_DVDT"},
    "R63-PWR1": {"1": "5V_USB_FILTERED", "2": "USB_EFUSE_EN"},
    "R2-PWR1": {"1": "USB_EFUSE_EN", "2": "USB_EFUSE_OVLO"},
    "R64-PWR1": {"1": "USB_EFUSE_OVLO", "2": "GND"},
    "R65-PWR1": {"1": "5V_PROTECTED", "2": "USB_EFUSE_PGTH"},
    "R66-PWR1": {"1": "USB_EFUSE_PGTH", "2": "GND"},
    "R67-PWR1": {"1": "3V3", "2": "USB_EFUSE_PG"},
    "C67-PWR1": {"1": "USB_EFUSE_DVDT", "2": "GND"},
}
...
def load_pin_map(base, live: dict[str, dict]) -> dict[str, dict[str, dict]]:
    ...
    ], "canonical-usb-efuse-support-pins")
...
def prepare() -> int:
    ...
    if missing:
        raise SystemExit(f"support components missing before wiring: {missing}")
    ...
    intended = (
        "Wire the USB/input TPS259474L support network in box 1, including UVLO/OVLO, "
        "PGTH, PG pull-up and dVdt, and visibly record the ITIMER no-connect intent"
    )
    base.begin_transaction(
        base.MUTATION_STATE, base.MUTATION_LEDGER, transaction_id=TX,
        project_uuid=PROJECT, document_uuid=PAGE, scope="POWER_ENTRY", stage="wire",
        kind="normal", intended_delta=intended, snapshot_path=snapshot,
        expected_checks=[
            "U1 pins EN OVLO PG PGTH and DVDT show the intended named nets",
            "every support resistor and capacitor shows two distinct endpoint nets",
            "the threshold and current settings note is visible and readable",
            "no duplicate wire stub overlap or unrelated component movement is visible",
        ],
    )
    ...
    remove_wire_ids = {"e1004", "e1006"}  # stale R2 wires left at its original position
    for record in all_rows:
        if record[0] != "WIRE":
            continue
        for segment in record[2]:
            if (int(segment[0]), int(segment[1])) in target_endpoints or (int(segment[2]), int(segment[3])) in target_endpoints:
    ...
    max_id += 1
    output.append(["TEXT", f"e{max_id}", 50, 3900, 0, NOTE, "st15", 0])
    head[1]["maxId"] = max_id

    if sum(record[0] == "COMPONENT" for record in output) != 189:
        raise SystemExit("wiring source changed the component count")
    if sum(record[0] == "RECT" for record in output) != 10:
        raise SystemExit("wiring source changed the domain rectangles")
    if sum(record[0] == "TEXT" and record[5] == NOTE for record in output) != 1:
        raise SystemExit("eFuse settings note is not unique")
    ...
    if observed_geometry != expected_geometry:
        raise SystemExit("prepared eFuse endpoint topology does not match the declared topology")
    payload = {
    ...
    (JOBS / f"{TX}-expected-geometry.json").write_text(json.dumps(expected_geometry, indent=2, sort_keys=True) + "\n")
...
def record() -> int:
    ...
    if observed_geometry != expected_geometry:
        raise SystemExit("live eFuse endpoint topology differs from the prepared topology")
    if sum(record[0] == "TEXT" and record[5] == NOTE for record in all_rows) != 1:
        raise SystemExit("live eFuse settings note is missing or duplicated")
    semantic = JOBS / f"{TX}-semantic.json"
    semantic.write_text(json.dumps({
        "schema_version": 1, "transaction_id": TX, "project_uuid": PROJECT,
        "document_uuid": PAGE, "scope": "POWER_ENTRY", "stage": "wire",
        "intended_delta": ("Wire the USB/input TPS259474L support network in box 1, including UVLO/OVLO, "
                           "PGTH, PG pull-up and dVdt, and visibly record the ITIMER no-connect intent"),
