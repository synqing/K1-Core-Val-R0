...
const XF = '0, 0, 0, 180, 0, 0, 0, 0, 0';
const OUT = 'evidence/VAL-G2-2026-08-28/canonical-core-val-r0/u1-gtusb-3d-rot180-bind-2026-08-30.json';
...
const result = await evaluate(`(async () => {
  ...
  try {
    out.modify = await R.pcb_PrimitiveComponent.modify(u1, {
      ...
      model3D: { libraryUuid: personal, uuid: orig, name: title },
    });
  } catch (e) {
...
writeFileSync(OUT, JSON.stringify({ recorded_at: new Date().toISOString(), original_uuid: ORIG, transform: XF, result }, null, 2));
