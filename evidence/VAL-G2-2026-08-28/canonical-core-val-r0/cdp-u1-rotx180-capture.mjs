...
const BASE = 'evidence/VAL-G2-2026-08-28/canonical-core-val-r0/u1-rotx180';
...
const pcb = await evaluate(`(async () => {
  ...
  return {
    ...
    title: other && other['3D Model Title'],
...
let zoomPt = null;
if (cropInfo.u1_css && c3d) zoomPt = { x: cropInfo.u1_css[0], y: cropInfo.u1_css[1] };
else if (c3d) zoomPt = { x: c3d.x + c3d.dw * 0.28, y: c3d.y + c3d.dh * 0.22 };
if (zoomPt) {
...
console.log(JSON.stringify({ ok: true, pcb, preview, refresh, top, fit, zoomPt, cropInfo, results }, null, 2));
ws.close();
