...
const OUT = process.argv[2]
  || 'evidence/VAL-G2-2026-08-28/canonical-core-val-r0/usb1-3d-after-y-autoz.png';
...
const closed = await evaluate(`(async () => {
  ...
  const tabs = [...document.querySelectorAll('[title],div,span')].filter(el => {
    const t = (el.getAttribute('title') || el.textContent || '').trim();
    return el.offsetParent !== null && (t === '3D PCB1' || t.startsWith('3D PCB'));
  });
  const out = { tabCount: tabs.length };
  if (tabs[0]) {
    tabs[0].dispatchEvent(new MouseEvent('click', { bubbles: true, cancelable: true, view: window }));
    await new Promise(r => setTimeout(r, 400));
    const cur = await R.dmt_SelectControl.getCurrentDocumentInfo();
    out.clicked = cur;
    if (cur && cur.documentType !== 3 && typeof R.dmt_EditorControl.closeDocument === 'function') {
      out.closed = await R.dmt_EditorControl.closeDocument(cur.tabId);
    }
  }
...
console.log(JSON.stringify({ ok: true, path: OUT, bytes: buf.length, closed, threeD }, null, 2));
ws.close();
