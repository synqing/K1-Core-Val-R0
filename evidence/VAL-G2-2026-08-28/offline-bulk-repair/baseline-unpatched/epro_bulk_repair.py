...
class Mutator:
    ...
    def apply(self):
        ...
        # RQ-025: observability net but physical RT endpoint deferred.
        self.set_pin_net('U1-PWR1','3','PWR_ENTRY_PG_RT_IOMUX_TBD',remove_existing=True)
        self.add_text(690,4370,'PWR_ENTRY_PG_RT_IOMUX_TBD: RT endpoint reserved for VAL-G3 pinmux')
        self.status('RQ-025','PARTIAL_G3','PG made explicit/observable net; RT ball deferred per VAL-G3 pinmux rule')
        ...
        self.status('RQ-035','APPLIED','external clock override no longer disables RT capture')
        ...
        # RQ-044/045 TPS2561 dual branch switch, schematic local symbol; remove U4 and branch ferrites from power-path identity.
        # Keep FB1/FB2 as post-switch EMI beads; replace common eFuse with TPS2561 between 5V_SYS and 5V_LED_COMMON sides.
        ...
        # remove old U4 eFuse only after building replacement branch nets
        ...
        # RQ-046/047 bindings. R1 already rebound above; R8 desired 3.48k device create using existing C185418 attrs from instance.
        d348=self.make_resistor_device('RC0402FR-073K48L','C185418','3.48kΩ')
        self.set_device('R8-PWR2',d348,self.project['devices'][d348]['title']+'.1');self.set_component_attr('R8-PWR2','Name','3.48k');self.set_component_attr('R8-PWR2','Manufacturer Part','RC0402FR-073K48L');self.set_component_attr('R8-PWR2','Supplier Part','C185418')
        self.status('RQ-046','APPLIED','R1 device binding now matches selected 1.24k part')
        self.status('RQ-047','APPLIED','R8 device binding now matches 3.48k drawn value')
